...
export function TaskList({ title = "All tasks", groupedTasks, disabled, onToggleTask }: TaskListProps) {
  return (
      ...
      <CardContent className="space-y-4">
        {groupedTasks.map((entry) => (
          <div key={entry.stage} className="space-y-3">
            <p className="text-sm font-medium uppercase tracking-[0.18em] text-ink/65">{formatStage(entry.stage)}</p>
